...
export default async function BillingPage() {
  ...
  if (!workspace) {
    return (
      <main className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <DashboardStatePanel eyebrow="Billing недоступен" title="Нет активного workspace" description="Создайте новый workspace и затем вернитесь сюда, чтобы включить SaaS billing, планы и Stripe flows." className="min-h-[380px]" />
  ...
  if (!summary) {
    return (
      <main>
        <DashboardStatePanel eyebrow="Billing" title="Workspace не найден" description="Не удалось загрузить billing context для текущего пользователя." className="min-h-[380px]" />
      </main>
    );
  }
    ...
    <main className="space-y-6">
      <section className="panel rounded-[32px] border border-white/10 px-6 py-6 sm:px-8">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-8 text-slate-300/80 sm:text-base">
          Stripe-powered billing surface для hosted SaaS mode: plan catalog, feature envelopes, Checkout, Customer Portal и webhook-driven sync статуса подписки обратно в PostgreSQL. На этом этапе лимиты уже применяются к write-операциям.
        </p>
      </section>
